...
class MuseumDBAnkoHelper(ctx: Context) : ManagedSQLiteOpenHelper(ctx, "MyDatabase", null, 1) {
    companion object {
        private var instance: MuseumDBAnkoHelper? = null
        ...
        @Synchronized
        fun getInstance(ctx: Context): MuseumDBAnkoHelper {
            if (instance == null) {
                instance = MuseumDBAnkoHelper(ctx.getApplicationContext())
            }
            return instance!!
        }
    }
...
val Context.database: MuseumDBAnkoHelper
    get() = MuseumDBAnkoHelper.getInstance(getApplicationContext())